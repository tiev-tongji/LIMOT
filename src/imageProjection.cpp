...
public:
    ImageProjection() : deskewFlag(0) {
        ...
        pubExtractedCloud = nh.advertise<sensor_msgs::PointCloud2>("limot/deskew/cloud_deskewed", 1);
        pubLaserCloudInfo = nh.advertise<limot::cloud_info>("limot/deskew/cloud_info", 1);
    ...
    void projectPointCloud() {
        ...
        for (int i = 0; i < cloudSize; ++i) {
            ...
            //Calculate the timestamp of each point, code from legoloam
            float relTime = 0;
            float startOrientation = -atan2(laserCloudIn->points[0].y, laserCloudIn->points[0].x);
            float endOrientation = -atan2(laserCloudIn->points[laserCloudIn->points.size() - 1].y, laserCloudIn->points[laserCloudIn->points.size() - 1].x) + 2 * M_PI;
            ...
            float scanPeriod = timeScanEnd - timeScanCur;
            ...
            if (deskewFlag == -1) {
                ...
                relTime = scanPeriod * (ori - startOrientation) / orientationDiff;
            }
